...
const ChevronIcon = ({ className }: SvgIconProps) => (
  <svg className={className} xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none" aria-hidden="true">
    <path
...
const KakaoIcon = ({ className }: SvgIconProps) => (
  <svg className={className} xmlns="http://www.w3.org/2000/svg" width="32" height="31" viewBox="0 0 32 31" fill="none" aria-hidden="true">
    <path
...
const MenuContent = ({ selectedMenuId, onMenuClick }: MenuContentProps) => (
  <>
...
const Faq = () => {
  ...
  const handleMenuClick = (menuId: string) => {
    ...
    if (menuId === 'all') {
      setOpenedItemId('');
      const firstSectionId = faqSections[0]?.id;
    ...
    const firstItem = faqSections.find((section) => section.id === menuId)?.items[0];
    if (firstItem) {
    ...
    <S.Page>
      ...
      <S.MobileMenu $isOpen={isMobileMenuOpen} aria-hidden={!isMobileMenuOpen}>
        <S.MobileMenuPanel ref={mobileMenuRef}>
          <MenuContent selectedMenuId={selectedMenuId} onMenuClick={handleMenuClick} />
        </S.MobileMenuPanel>
                  ...
                  {section.items.map((item) => {
                    ...
                    return (
                      <S.AccordionItem key={item.id} $isOpen={isOpen}>
                        <S.AccordionButton type="button" onClick={() => handleAccordionClick(item.id, section.id)} aria-expanded={isOpen}>
                          <span>{getHighlightedText(item.question, submittedSearchQuery)}</span>
                          <S.Chevron as={ChevronIcon} />
                        </S.AccordionButton>
                        <S.AccordionPanel $isOpen={isOpen} $isInstant={isMenuScrolling} aria-hidden={!isOpen}>
                          <S.AccordionPanelInner $isOpen={isOpen} $isInstant={isMenuScrolling}>
