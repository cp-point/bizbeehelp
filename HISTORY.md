...
- 모바일 터치 환경에서는 FAQ 아코디언 타이틀의 hover 색상 및 배경 효과가 적용되지 않도록 수정

## 2026-07-28 10:00

- 메인 FAQ 화면을 일반 스크롤할 때 현재 위치의 소분류 메뉴가 좌측 메뉴에서 자동으로 선택되도록 동기화

## 2026-07-09 10:10

- 메인 FAQ 헤더 아래 전체 스크롤바 스타일을 왼쪽 분류 메뉴 스크롤바와 동일하게 조정

## 2026-07-09 10:05

- 메인 FAQ 화면에서 오른쪽 전체 스크롤이 상단 헤더 아래 콘텐츠 영역에만 적용되도록 PageScroll 구조를 추가

## 2026-07-09 09:55

- 메인 FAQ 오른쪽 FAQ 목록 영역에 적용된 내부 스크롤 컨테이너를 제거

## 2026-07-09 09:35

- 메인 FAQ 왼쪽 분류 메뉴에서 전체 버튼 아래 대분류/소분류 목록 영역에만 스크롤이 적용되도록 수정
...
- 메인 FAQ 검색창의 기본 X 버튼으로 검색어를 지우면 검색 조건도 함께 초기화되어 전체 목록이 다시 보이도록 수정

## 2026-07-09 08:50

- 메인 FAQ 페이지 중심으로 크롤링을 허용하고 관리자/API 경로는 제외하도록 robots.txt 작성

## 2026-07-09 08:42

- `/admin/login` 페이지를 서버 쿠키 기준으로 리다이렉트하도록 분리해 로컬 저장소에 남은 로그인 정보 때문에 로그인 화면 접근이 막히는 문제 수정

## 2026-07-07 10:37

- Tiptap Link 팝오버 입력란이 주간 모드에서 흰색으로 보이지 않도록 입력창 색상 스타일 수정

## 2026-07-07 10:35

- Tiptap SimpleEditor 주간 모드에서 링크/링크 입력창 텍스트가 보이도록 색상 변수를 명시적으로 조정

## 2026-07-06 17:30

- FAQ 목록/등록/분류관리 화면에서 컴포넌트 내부에 있던 도메인 타입을 `types/Faq.ts`로 분리
- 공통 Table 컴포넌트의 컬럼/props 타입을 `types/Table.ts`로 분리하고 참조 import 정리

## 2026-07-06 16:38

- FAQ분류관리 대분류/소분류 저장 기준을 `majorId`, `minorId` 기반으로 변경해 코드 수정 시 신규 생성되지 않고 기존 행이 수정되도록 반영
- 대분류/소분류 코드 중복 입력 시 저장 전에 중단하도록 프론트 검증 추가
- 백엔드 분류 DTO, 엔티티, 레포지토리, 서비스 로직을 ID PK와 code unique 구조에 맞게 수정

## 2026-07-03 10:02

- `/faq` 검색 결과가 있는 키워드만 `/popular`로 저장 요청을 보내도록 연결
- 인기 검색어 저장 시 기존 키워드는 조회수를 증가시키고 신규 키워드는 새로 추가하는 백엔드 API 추가
- `/faq` 추천 검색어를 `GET /popular` 상위 5개 결과로 표시하고 데이터가 없으면 기본 키워드를 표시하도록 수정

## 2026-07-02 13:21

- 로그인된 상태에서 `/admin/login` 접근 시 로그인 완료 화면을 표시하지 않고 `/admin/faqs`로 바로 이동하도록 수정
- 로그인 페이지의 깨진 한글 문구를 정상 문구로 정리

## 2026-07-02 10:46

- `/faq` 메인 화면의 카카오톡 문의하기 플로팅 버튼 클릭 시 카카오톡 채팅 링크가 새 탭으로 열리도록 연결
...
- `/faq` 관련 사이트 목록을 사이트관리 `siteUrl` API에서 POST로 조회해 사용 중인 항목을 순번 기준으로 표시하도록 수정
- 사이트관리 API 실패 또는 빈 응답 시 기존 정적 푸터/관련사이트 데이터를 fallback으로 사용하도록 처리
- 사이트관리 API 값이 빈 값일 때 기존 정적 푸터/관련사이트 값으로 대체하지 않고 빈 항목은 표시하지 않도록 수정

## 2026-07-02 08:58

- FAQ분류관리 소분류 탭 테이블에 대분류명 선택 컬럼을 추가
- 소분류 추가 시 첫 대분류(D1)가 자동 지정되지 않도록 하고, 선택한 대분류 코드가 `/minor/upsert` payload에 포함되도록 수정
- 소분류 탭 컬럼 추가에 맞춰 분류관리 테이블 최소 너비 조정
- FAQ분류관리 테이블의 분류명/대분류 선택값이 가운데 정렬되도록 수정
...
- metadata description 한글 문구를 정상화하고 디버그 로그를 제거
